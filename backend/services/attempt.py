...
class QuizAttemptService(BaseService[QuizAttempt]):
    ...
    def create_quiz_attempt(self, quiz_id: int, attempt_create: QuizAttemptCreate):
        ...
        for answer in attempt_create.answers:
            ...
            question = [q for q in quiz.questions if q.id == answer.question_id]
            ...
            invalid_answer = user_answer - set(question[0].options.keys())
            ...
            correct_answer = set(question[0].correct_answer)

            if user_answer == correct_answer:
                valid_answers += 1

        final_score = round((valid_answers / len(quiz.questions)) * 100, 2)

        quiz_attempt = QuizAttempt(score=final_score, quiz_id=quiz_id)
    ...
    def get_quiz_attempts_by_quiz(self, quiz_id: int):
        return (
            self.session.query(QuizAttempt).where(QuizAttempt.quiz_id == quiz_id).all()
        )

    def get_quiz_attempt_by_quiz_and_id(self, quiz_id: int, attempt_id: int):
        quiz_attempt = self.session.scalar(
            select(QuizAttempt).where(
                QuizAttempt.quiz_id == quiz_id and QuizAttempt.id == attempt_id
            )
        )
